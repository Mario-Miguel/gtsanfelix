    ...
    <header v-if="!isAdmin"
      ...
      <div v-if="menuOpen" class="md:hidden bg-[#C2B280]/5 border-t border-[#C2B280]/20 px-6 pb-4">
        <ul class="flex flex-col gap-4 pt-4 text-sm font-medium">
          <li>
            <RouterLink to="/" @click="menuOpen = false" class="block hover:text-[#C2B280]">Iniciu</RouterLink>
          </li>
          <li>
            <RouterLink to="/about" @click="menuOpen = false" class="block hover:text-[#C2B280]">Compañía</RouterLink>
          </li>
          <li>
            <RouterLink to="/repertoire" @click="menuOpen = false" class="block hover:text-[#C2B280]">Repertoriu
            </RouterLink>
          </li>
          <li>
            <RouterLink to="/calendar" @click="menuOpen = false" class="block hover:text-[#C2B280]">Cartelera
            </RouterLink>
          </li>
          <li>
            <RouterLink to="/contact" @click="menuOpen = false" class="block hover:text-[#C2B280]">Contactu</RouterLink>
          </li>
        </ul>
      </div>
    </header>
